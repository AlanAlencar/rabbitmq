 ...
 *              mensagens roteadas pela routing key "success".
...
(async () => {
    ...
    const myQueue = await channel.assertQueue('queueSuccess');
    /**
     * @param queue Queue name (string);
     * @param exchange Exchange name (string);
     * @param pattern Routing key [direct] (string);
     */
    await channel.bindQueue(myQueue.queue, config.rabbitMQ.exchangeName, 'success');
