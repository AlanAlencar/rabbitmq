 ...
 *              mensagens roteadas pela routing key "fail".
...
(async () => {
    ...
    const myQueue = await channel.assertQueue('queueFail');
    /**
     * @param queue Queue name (string);
     * @param exchange Exchange name (string);
     * @param pattern Routing key [direct] (string);
     */
    await channel.bindQueue(myQueue.queue, config.rabbitMQ.exchangeName, 'fail');
